...
(function() {
  var $, Constraint, GraphDatabase, Index, Neo4jError, Node, Relationship, Request, Transaction, URL, assert, lib, _normalizeAuth, _transform,
    __slice = [].slice;
  ...
  Neo4jError = require('./errors').Neo4jError;
  ...
  module.exports = GraphDatabase = (function() {
    ...
    GraphDatabase.prototype.cypher = function(opts, cb, _tx) {
      ...
      if (!_tx && rollback) {
        throw new Neo4jError('Illegal state: rolling back without a transaction!');
      }
      if (commit && rollback) {
        throw new Neo4jError('Illegal state: both committing and rolling back!');
      }
      if (rollback && (query || queries)) {
        throw new Neo4jError('Illegal state: rolling back with query/queries!');
      }
      if (!_tx && commit === false) {
